...
// FIELD BY VENUE
// ==========================
Route::get('/venues/{id}/fields', fn ($id) => view('field.index', compact('id')))
    ->name('venues.fields');

Route::get('/fields/{id}', fn ($id) => view('field.detail', compact('id')))
    ->name('fields.detail');
...
Route::get('/payment/qr', fn () => view('booking.qr'))->name('payment.qr');
Route::get('/payment/success', fn () => view('booking.success'))->name('payment.success');

Route::post('/matches/{id}/join', [MatchController::class, 'join'])->name('match.join');

// ==========================
// USER AREA (FIX UTAMA DI SINI)
// ==========================
Route::prefix('user')
    ->name('user.')
    ->middleware('auth')
